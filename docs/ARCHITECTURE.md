...
## Sandbox Observatory (`engine/retrial/registry.py`)

A thread-safe `SandboxRegistry` (process-default `REGISTRY`, attached to the bus in `server.py`) is the single ledger of every sandbox the system touches. `SandboxPool`, `ForkSandboxPool`, `FlakeBisector`, and `trial.py` call `@_safe`-wrapped hooks at create/fork/pause/start/exec/destroy points — so a broken or absent registry can never break a run (proven by the `RaisingRegistry` tests). Each record carries role, state, fork-lineage parent, the running command, a bounded ring of recent execs, an exec count, and a lazily-resolved Daytona preview link (positive results cache forever; a paused-sandbox failure is retried, never poisoned; `None` once destroyed). It emits `sandbox_registered` / `sandbox_state` / `sandbox_exec` / `sandbox_destroyed` / `registry_snapshot` over `/ws`, and backs `GET /sandboxes`, `GET /sandboxes/{id}`, `DELETE /sandboxes/{id}`, and `POST /sandboxes/destroy_all` (409 while a run is active unless `?force=1`; force cooperatively **cancels** an active bisector rather than yanking its resources). The registry is never reset at run acceptance — live sandboxes and the exact total-ever/destroyed counters span runs (the pool is shared) — but `_accept_run()` re-emits a fresh `registry_snapshot` after `BUS.reset()` so a mid-run subscriber sees the real sandbox world. Destroyed records are bounded to the `RETRIAL_DESTROYED_RETAIN` most recent while the counters stay exact.

**Spend meter (T1-4):** the registry also accumulates exact-forever sandbox-lifetime seconds (created→destroyed on its monotonic clock; pruning a destroyed record never loses seconds) and `snapshot()` carries a `spend` object — live/total sandbox-seconds plus a clearly-labeled `est.` cost from `RETRIAL_EST_RATE_PER_SANDBOX_HOUR` (`null` when unset). Not Daytona billing data; no fake precision. Surfaced in `GET /sandboxes` and a small honest chip in the Observatory header.

**Loud degrade banner (T1-1 UI):** `DegradeBanner` renders at the board root (covers every view) from two sticky pool-level facts — `pool_degraded` and a failed boot `preflight_done`, both re-seeded through `BUS.reset()` at run acceptance — as a persistent red banner that never auto-dismisses; warn-only preflight checks render a slim amber strip. Silent degrade on stage is the disaster it prevents.

**Run history (T2-4, `engine/retrial/history.py`):** a stdlib-`sqlite3` (no ORM) single-table ledger of completed runs at `RETRIAL_DB`; `record` is `@_safe`-wrapped and called from the server's run `finally` behind a second call-site guard (a history failure costs the row, never the verdict); `recent` returns `[]` on any failure. WAL + `busy_timeout` keep `GET /runs` non-blocking. Surfaced at `GET /runs` (read-only) and a live-only, read-only `RunHistory` UI panel — deliberately outside the event stream, so the sacred replay is unaffected by construction.
