...
| `RETRIAL_PREVIEW` | `0` | pool / trial | `1` = pool sandboxes are created **public** and serve a live status page, so preview links open in any browser. See **Shareable previews** below |
| `RETRIAL_PREVIEW_PORT` | `8080` | registry / pool | port the preview page is served on and resolved against |
...
### Shareable previews (`RETRIAL_PREVIEW=1`, default off)

Every sandbox in the Observatory can expose a **live preview URL that opens in an
ordinary browser tab** — no API key, no token, no extension. Off by default,
because turning it on publishes those sandboxes to anyone holding the link.

Three things were in the way, all measured on live Daytona (2026-07-25):

| | Result |
|---|---|
| `get_preview_link(port)` on a container | **works** — a Linux VM is *not* required |
...
- each trial rewrites the page with its own real exit code, inside the same exec
  that runs the test.

The page states only what was measured: this sandbox's last exit code, and what
0 and 1 mean. `GET /sandboxes/{id}` returns the URL, and the Observatory drawer's
preview button opens it.

The token requirement is why an embedded `<iframe src>` cannot show a private
preview — a tab and an iframe both send plain GETs. `public=True` is the fix, and
it is the reason this is opt-in rather than default.
